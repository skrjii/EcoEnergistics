...
public class RenderEcoTransmitterBase <T extends TileEntityEcoTransmitter> extends TileEntitySpecialRenderer<T> {
    ...
    public RenderEcoTransmitterBase() {
        ...
        if (!keys.isEmpty()) {
            for (String key : keys) {
                if (!modelParts.containsKey(key)) {
                    modelParts.put(key, objModel.bake(new OBJModel.OBJState(Collections.singletonList(key), false), Attributes.DEFAULT_BAKED_FORMAT, textureGetterFlipV));
                }
            }
        }
        return modelParts;
    ...
    public IBakedModel getModelForSide(TileEntityEcoTransmitter part, EnumFacing side) {
