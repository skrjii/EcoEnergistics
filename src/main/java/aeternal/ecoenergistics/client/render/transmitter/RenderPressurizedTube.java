...
import aeternal.ecoenergistics.common.tile.transmitter.TransmitterEImpl;
...
public class RenderPressurizedTube extends RenderTransmitterSimple<TileEntityEcoPressurizedTube> {
    ...
    public void render(TileEntityEcoPressurizedTube tube, double x, double y, double z, float partialTick, int destroyStage, float alpha) {
        if (!MekanismConfig.current().client.opaqueTransmitters.val()) {
            TransmitterEImpl<IGasHandler, GasNetwork, GasStack> transmitter = tube.getTransmitter();
