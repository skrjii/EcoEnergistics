...
public class TransmitterEImpl<ACCEPTOR, NETWORK extends DynamicNetwork<ACCEPTOR, NETWORK, BUFFER>, BUFFER> extends Transmitter<ACCEPTOR, NETWORK, BUFFER> {
    ...
    public TransmitterEImpl(TileEntityEcoTransmitter<ACCEPTOR, NETWORK, BUFFER> multiPart) {
    ...
    public boolean isCompatibleWith(IGridTransmitter<ACCEPTOR, NETWORK, BUFFER> other) {
        if (other instanceof TransmitterEImpl) {
            return containingTile.isValidTransmitter(((TransmitterEImpl) other).containingTile);
        }
