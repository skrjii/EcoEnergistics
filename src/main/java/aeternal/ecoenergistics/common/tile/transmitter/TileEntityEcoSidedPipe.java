    ...
    public List<AxisAlignedBB> getCollisionBoxes() {
        ...
        for (EnumFacing side : EnumFacing.VALUES) {
            ...
            if (connectionMapContainsSide(connections, side)) {
                list.add(getTransmitterType().getSize() == Size.SMALL ? BlockEcoTransmitter.smallSides[ord] : BlockEcoTransmitter.largeSides[ord]);
            }
        }
        list.add(getTransmitterType().getSize() == Size.SMALL ? BlockEcoTransmitter.smallSides[6] : BlockEcoTransmitter.largeSides[6]);
        return list;
    }

    public abstract EcoTransmitterType getTransmitterType();
        ...
        for (EnumFacing side : EnumFacing.VALUES) {
            if (connectionMapContainsSide(newlyEnabledTransmitters, side)) {
                ...
                if (tileEntity instanceof TileEntityEcoSidedPipe) {
                    ((TileEntityEcoSidedPipe) tileEntity).refreshConnections();
                }
            }
        }
    }
