...
public class RenderSolarStationInfinity extends TileEntitySpecialRenderer<TileEntitySolarStationInfinity> {

    private ModelSolarStationAvaritia model = new ModelSolarStationAvaritia();

    @Override
    public void render(TileEntitySolarStationInfinity tileEntity, double x, double y, double z, float partialTick, int destroyStage, float alpha) {
        GlStateManager.pushMatrix();
        GlStateManager.translate((float) x + 0.5F, (float) y + 1.5F, (float) z + 0.5F);
        bindTexture(Constants.getResource(Constants.ResourceType.RENDER, "InfinitySolarStation.png"));
